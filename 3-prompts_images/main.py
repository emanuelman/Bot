...
def main(profile_ideas, profile_prompts, driver_ideas, driver_prompts, url_ideas, url_prompt, titulo, introducao, primeiro_paragrafo, segundo_paragrafo, terceiro_paragrafo, quarto_paragrafo, finalizacao):
    profiles = []
    ...
    # Verificar se algum prompt está vazio ou `None`
    for idx, prompt in enumerate(prompts):
        if not prompt:
            print(f"Erro: O prompt número {idx + 1} está vazio ou não definido corretamente.")
            #return
        else:
            print(f"O prompt número {idx + 1} está correto.")

    # Se todos os prompts estiverem corretos, continue

    # Criar uma instância de GenerateImagePrompts passando os prompts
    instance = GenerateImagePrompts(driver_ideas, driver_prompts, titulo, introducao, primeiro_paragrafo, segundo_paragrafo, terceiro_paragrafo, quarto_paragrafo, finalizacao)

    # Chamar o método generate_prompts da instância
    instance.generate_prompts()  # Chamar método da instância

if __name__ == '__main__':
    main(profile_ideas, profile_prompts, driver_ideas, driver_prompts, url_ideas, url_prompt, titulo, introducao, primeiro_paragrafo, segundo_paragrafo, terceiro_paragrafo, quarto_paragrafo, finalizacao)
